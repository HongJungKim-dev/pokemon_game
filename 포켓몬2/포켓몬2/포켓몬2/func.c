...
void set_player_info(Player *player, Pokemon ***pokemon_book_addr) {
	...
	//	pokemon = make_pokemon(player->battle_pokemon+i);

	//	//포켓몬 공간을 플레이어 공간에 연결
	//	(player->battle_pokemon)[i] = *pokemon;
	//}
...
void set_pokemon_info(Player* player, Pokemon ***pokemon_book_addr) {
	...
	case ggobugi:
		...
		player->battle_pokemon->name = (*pokemon_book_addr)[0][0].name[0];
	...
	case pichu:
		...
		//name
		player->battle_pokemon->name = (*pokemon_book_addr)[3][0].name[0];
		//max_hp (초기 hp와 동일)
	...
	case fire:
		...
		//name
		player->battle_pokemon->name = (*pokemon_book_addr)[1][0].name[0];
		//max_hp (초기 hp와 동일)
...
/*
// 포켓몬 도감 column 만들기
int makeCol2(Skill ***p_addr, int col) {
	*p_addr = (Skill **)malloc(sizeof(Skill *) * col);

	if (*p_addr == NULL) {
		printf("할당 실패\n");
		return 1;
	}
	memset(*p_addr, 0x0, sizeof(Skill *) * col);
	return 0;
}


// 포켓몬 도감 row 만들기
int makeRow2(Skill **p_addr, int row) {
	*p_addr = (Skill *)malloc(sizeof(Skill) * row);

	if (*p_addr == NULL) {
		printf("할당 실패\n");
		return 1;
	}
	memset(*p_addr, 0x0, sizeof(Skill) * row);
	return 0;
}
*/
...
//	for (i = 0; i < 5; i++) {
//		if (i == 4) {
//			malloc_fail = makeRow2(*p_addr + i, 1);
//			(*(*p_addr + i))[0].skill1 = (Skill *)malloc(sizeof(Skill) * 20);
//			(*(*p_addr + i))[0].skill2 = (Skill *)malloc(sizeof(Skill) * 20);
//			(*(*p_addr + i))[0].skill3 = (Skill *)malloc(sizeof(Skill) * 20);
//		}
//		else {
//			malloc_fail = makeRow(*p_addr + i, 4);
//			if (malloc_fail == 1) {
...
//			}
//			else for (j = 0; j<4; j++) {
//				(*(*p_addr + i))[j].skill1 = (Skill *)malloc(sizeof(Skill) * 20);
//				(*(*p_addr + i))[j].skill2 = (Skill *)malloc(sizeof(Skill) * 20);
//				(*(*p_addr + i))[j].skill3 = (Skill *)malloc(sizeof(Skill) * 20);
//			}
//		}
//
//	}
//	f1 = fopen("C:\\toy\\skill.txt", "r");
//	if (f1 != NULL) {
//		while (!feof(f1)) {
//			fscanf(f1, "%s", (*p_addr)[k][l].skill1);
//			fscanf(f1, "%s", (*p_addr)[k][l].skill2);
//			fscanf(f1, "%s", (*p_addr)[k][l].skill3);
//			l++;
//			if (k == 4) l = 0;
//			else if (l == 4) {
//				k++;
